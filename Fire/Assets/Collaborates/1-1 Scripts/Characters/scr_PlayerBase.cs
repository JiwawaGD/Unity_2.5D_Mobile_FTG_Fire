...
public class scr_PlayerBase : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision col)
    {
        ...
        else if (col.gameObject.tag.Contains(enemyAttack))
        {
            if (col.gameObject.name.Contains("普攻碰撞器") || col.gameObject.name.Contains("尖刺碰撞器"))
            {
                Hurt(10);
            }
            else if (col.gameObject.name.Contains("技能1碰撞") || col.gameObject.name.Contains("電流碰撞器"))
            {
                Hurt(20);
            }
            else if (col.gameObject.name.Contains("技能2碰撞"))
            {
                Hurt(20);
            }
        }
